...
def validate_screen_positions(pos):
    """
    Ensures a position is within the confines of a stnadard 4k resoltuion screen

    This function is designed so that it can be used in the same place as the pos parameter
    It should simply be placed where the pos wants to be used
    It will quit the program if the pos is invalid

    :param pos: he position to check
    :type pos: tuple[int, int]
    :return: The position if it is valid
    :rtype: tuple[int, int]
    """
    if pos[0] < 0 or pos[0] > 3840 or pos[1] < 0 or pos[1] > 2160:
        print("Error: invalid x or y value has been calculted.")
        pygame.quit()
        sys.exit()
    else:
        return pos
        ...
        if row_col:  # rotate row
            ...
            if number == 0:
                used_cube[4] = numpy.rot90(
                    used_cube[4], k=1, axes=(1, 0)
                )  # rotate the top face
            elif number == 2:
                used_cube[5] = numpy.rot90(
                    used_cube[5], k=1, axes=(0, 1)
